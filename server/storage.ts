...
export class DatabaseStorage implements IStorage {
  ...
  async seedDefaultScripts(): Promise<void> {
    const defaultUpdateScripts: Record<string, { command: string; description: string; fileExtension: string }> = {
      ...
      juniper: { command: `# Script de Atualizacao Juniper Junos\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\nfile copy http://{{SERVER_IP}}/firmware/{{FIRMWARE_FILE}} /var/tmp/\nrequest system software add /var/tmp/{{FIRMWARE_FILE}} no-validate\nrequest system reboot`, description: "Script padrao de atualizacao via FTP para roteadores Juniper", fileExtension: ".tgz" },
      fortinet: { command: `# Script de Atualizacao Fortinet FortiGate\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\nexecute restore image tftp {{FIRMWARE_FILE}} {{SERVER_IP}}\ny`, description: "Script padrao de atualizacao via TFTP para firewalls Fortinet", fileExtension: ".out" },
      ubiquiti: { command: `# Script de Atualizacao Ubiquiti EdgeOS\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\nadd system image http://{{SERVER_IP}}/{{FIRMWARE_FILE}}\nreboot`, description: "Script padrao de atualizacao via HTTP para Ubiquiti EdgeOS", fileExtension: ".tar" },
      intelbras: { command: `# Script de Atualizacao Intelbras\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\ncopy tftp://{{SERVER_IP}}/{{FIRMWARE_FILE}} flash:\nboot system flash:{{FIRMWARE_FILE}}\nreload`, description: "Script padrao de atualizacao para switches Intelbras", fileExtension: ".bin" },
      "hp-aruba": { command: `# Script de Atualizacao HP/Aruba\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\ncopy tftp flash {{SERVER_IP}} {{FIRMWARE_FILE}} primary\nreload`, description: "Script padrao de atualizacao para switches HP/Aruba", fileExtension: ".swi" },
      ubiquiti: { command: `# Script de Atualizacao Ubiquiti EdgeOS\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\nadd system image http://{{SERVER_IP}}/{{FIRMWARE_FILE}}\nreboot`, description: "Script padrao de atualizacao via HTTP para Ubiquiti EdgeOS", fileExtension: ".tar" },
      intelbras: { command: `# Script de Atualizacao Intelbras\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\ncopy tftp://{{SERVER_IP}}/{{FIRMWARE_FILE}} flash:\nboot system flash:{{FIRMWARE_FILE}}\nwrite\nreload`, description: "Script padrao de atualizacao via TFTP para switches Intelbras", fileExtension: ".bin" },
      "hp-aruba": { command: `# Script de Atualizacao HP/Aruba\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\ncopy tftp flash {{SERVER_IP}} {{FIRMWARE_FILE}} primary\nboot system flash primary\nwrite memory\nreload`, description: "Script padrao de atualizacao via TFTP para switches HP/Aruba", fileExtension: ".swi" },
      grandstream: { command: `# Script de Atualizacao Grandstream via Provisioning\n# Placeholders: {{SERVER_IP}}, {{FIRMWARE_FILE}}, {{EQUIPMENT_IP}}\n# Grandstream usa provisionamento HTTP para atualizacao\nconfig server path http://{{SERVER_IP}}/firmware\nconfig server upgrade 1\nconfig server upgrade filename {{FIRMWARE_FILE}}\nreboot`, description: "Script padrao de atualizacao via provisionamento para equipamentos Grandstream", fileExtension: ".bin" },
    };
    ...
    const defaultBackupScripts: Record<string, { command: string; description: string; fileExtension: string }> = {
      ...
      dell: { command: `# Script de Backup Dell PowerSwitch\n# Placeholders: {{EQUIPMENT_IP}}\nshow running-configuration`, description: "Script padrao de backup para switches Dell", fileExtension: ".cfg" },
